...
// Inicializar la IA de forma perezosa para evitar errores al inicio si falta la API Key
let aiInstance = null;
...
const getAi = () => {
  if (!aiInstance) {
    ...
    if (!apiKey) {
      console.warn("GEMINI_API_KEY no encontrada. Las funciones de IA estarán desactivadas.");
      return null;
    }
...
export const parseTaskWithGemini = async (command, availableParentTasks) => {
  ...
  const model = "gemini-2.5-flash"; // Usamos Flash por ser rápido y eficiente
  const now = new Date();
