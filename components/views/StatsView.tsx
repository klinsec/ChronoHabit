...
const StatsView: React.FC = () => {
    ...
    if (!firebaseUser) {
        return (
            <div className="flex flex-col items-center justify-center h-full p-8 space-y-6 text-center animate-in fade-in">
    ...
    return (
        <div className="h-full flex flex-col p-2 space-y-4">
            {/* Header Stats */}
            <div className="bg-surface p-4 rounded-2xl shadow-lg border border-primary/20 flex items-center justify-between">
                <div>
                    <p className="text-xs text-gray-400 uppercase tracking-widest">Tu Puntuación</p>
                    <h2 className="text-3xl font-bold text-primary">{myScore}</h2>
                </div>
            ...
            {/* Ranking List */}
            <div className="flex-grow bg-surface rounded-2xl p-4 shadow-lg border border-gray-800 overflow-hidden flex flex-col">
                <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                    <StarIcon /> Tabla de Líderes
                </h3>
                
                {rankingError ? (
                    <div className="text-red-400 text-center text-sm p-4 bg-red-900/10 rounded">
                        Error cargando ranking: {rankingError}
                    </div>
                ) : (
                    <div className="overflow-y-auto space-y-2 pr-1">
                        {leaderboard.length === 0 ? (
                            <p className="text-center text-gray-500 py-4">Cargando clasificación...</p>
                        ) : (
                            leaderboard.map((user, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className={`w-8 h-8 flex items-center justify-center font-bold rounded-full mr-3 ${index < 3 ? 'text-black' : 'text-gray-400 bg-gray-700'}`}
                                            style={{ backgroundColor: index === 0 ? '#FFD700' : index === 1 ? '#C0C0C0' : index === 2 ? '#CD7F32' : undefined }}
                                        >
                                            {index + 1}
                                        </div>
                                        <div className="flex-grow flex items-center gap-2">
                                            {user.photo && <img src={user.photo} className="w-6 h-6 rounded-full" alt="" />}
